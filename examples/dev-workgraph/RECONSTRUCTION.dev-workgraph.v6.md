...
I designed and implemented `dev-workgraph-cli`, an open-source developer tool that reconstructs a professional's contributions by synthesizing Git history, patches, and human-provided context into an evidence-based career narrative. I scoped the tool as a local, point-in-time analysis utility rather than a shared organizational knowledge graph, focusing on reproducible reconstruction. To ensure this reproducibility, I implemented schema versioning and stamped artifacts, allowing every generated report to be traced back to the specific pipeline version, source report, and model provenance that produced it. This architecture ensures that as the data format evolves, older reports remain understandable or migratable.

I architected the system to maintain a strict separation between deterministic data—such as commits, file paths, and timestamps—and model-generated layers. This prevents stable evidence from being overwritten by narratives that may evolve as models are updated. To ensure the work graph reflects architectural evolution rather than just commit volume, I implemented area detection and noise filtering algorithms. These treat maintenance activity—specifically dependency updates, routine upkeep, and application release commits—as secondary signals, preventing them from dominating the reconstruction of architectural decisions or subsystem boundaries.

During implementation, I discovered that Git history alone was insufficient to capture design ownership, leading me to pivot the MVP into a structured evidence pipeline. This pipeline converts raw data into technical, architecture, or security signals, which are ranked by importance (high, medium, and low context). To bridge the gap between raw evidence and role-based interpretation, I implemented a feedback loop: the system analyzes Git evidence to identify gaps in intent or reasoning, generates targeted questions for the user, and incorporates the resulting answers as a separate context layer. This process allows human input to reframe factual Git data into claims of ownership and impact without overriding the deterministic evidence.

To support this synthesis, I introduced a dedicated `narrativeModel` slot, decoupling narrative generation from reporting logic. While the report model handles structured technical analysis, the narrative model translates confirmed signals and human-corrected context into defensible outputs, such as role summaries and CV bullets. To make these outputs effective for external review and automated screeners, I implemented an evidence-based normalization layer via prompt-driven heuristics. This maps internal project-specific jargon to standardized industry technical keywords based on detected signals, ensuring that implementation details are framed in terms of broader concepts like provenance tracking or LLM pipeline architecture.

I reinforced the system's operational stability by integrating a `TokenUsageTracker` for local resource visibility and implementing a comprehensive Vitest suite across all primary CLI actions. To prevent architectural drift during rapid iteration, I automated UML diagram generation within the pre-commit workflow, ensuring visual documentation remains synchronized with the code. Finally, I productized the tool as an open-source npm package, prioritizing a local-first security model where Git history and LLM processing via Ollama remain on the developer's machine by default.

## Technologies

TypeScript, Git, Node.js, Ollama, Drizzle ORM

## Impact bullet points (Role Narrative)

- I designed and implemented dev-workgraph-cli, an open-source tool that reconstructs developer contributions by synthesizing Git history, patches, and human context into a reproducible career narrative.
- I architected the evidence pipeline to maintain a strict separation between deterministic data—such as commits and file paths—and model-generated layers, ensuring stable evidence is not overwritten by evolving narratives.
- I implemented schema versioning and stamped artifacts to ensure local reproducibility, allowing generated reports to be traced back to specific pipeline versions, source reports, and model provenance.
- I developed a noise filtering and area detection system that treats maintenance activity, such as dependency updates and release commits, as secondary signals to prevent them from dominating the reconstruction of architectural decisions.

## CV bullets

- Designed and implemented dev-workgraph-cli, an open-source tool synthesizing Git history, patches, and human context into reproducible career narratives.
- Architected a multi-layer evidence pipeline separating deterministic Git data from model-generated layers to ensure factual grounding and provenance.
- Implemented schema versioning and stamped artifacts for local reproducibility, enabling traceability across pipeline versions, source reports, and model provenance.
- Developed noise filtering and area detection algorithms to isolate architectural signals from routine maintenance activity, ensuring accurate design ownership reconstruction.
...
**Q:** (missing question 1782633938788)
**A:** (no answer)

**Q:** (missing question 1782633938789)
**A:** (no answer)

**Q:** While you've clarified that schema versioning and stamps are for local reproducibility, do you envision these stamped artifacts serving as 'proof-of-work' tokens that can be shared with managers or peers to validate a reconstruction's provenance during a review, or are they intended strictly as private metadata?
**A:** The stamped artifacts are not intended to be formal proof-of-work tokens.

The schemaVersion exists mainly for compatibility and migrations. As the reconstruction format evolves, older generated reports may have a different JSON structure. Schema versioning allows the tool to understand which format it is reading and, when needed, support migrations or backward-compatible handling of older artifacts.

The TokenUsageTracker has a different purpose. It is local operational statistics. It helps the user understand how many tokens were spent during the analysis, which stages consumed them, and how expensive a reconstruction run was. This is useful for local control, debugging, and optimization, especially when running different models or repeating the analysis.

So these fields are private technical metadata by default. They help the tool remain maintainable and reproducible locally, but they are not designed as cryptographic proof or as a formal review attestation mechanism.

That said, the metadata can still support transparency. If a developer shares a reconstruction with a manager or peer, the schema version, source report, model name, generation timestamp, and token usage can help explain how the artifact was produced. But the real evidence is still the Git history and source patches. The metadata describes the processing context; it does not replace the evidence itself.

In short, schema versioning supports artifact migrations, TokenUsageTracker supports local usage visibility, and Git remains the factual source of truth.

**Q:** Given the transition to an OSS tool that processes potentially sensitive Git history through local LLMs, have you defined any security boundaries or sanitization layers in the evidence pipeline to prevent sensitive data from being persisted in local model caches or logs during the summarization process?
**A:** The primary security boundary is that dev-workgraph-cli is local-first.

By default, the tool runs on the developer’s machine and uses a local LLM runtime such as Ollama. The Git history, patches, prompts, model responses, and generated reconstruction artifacts stay in the local environment. Nothing is sent to an external model provider by default.

Because of that, the main design decision is not to build the tool around a cloud AI service. The evidence pipeline is intended to operate locally over the repository, with the local model producing summaries from local Git evidence.

If a user chooses to configure a cloud model or an external LLM provider, that becomes an explicit user decision and a different trust boundary. In that case, the user or organization is responsible for deciding whether their repository content is allowed to be sent to that provider.

So I would frame the security model as: local execution by default, no external data egress by default, and user-controlled model configuration. Sanitization or redaction can still be added as a hardening layer, but the core boundary is that the default workflow keeps sensitive Git history local.

**Q:** Regarding the translation of internal jargon into standardized technical keywords: is this mapping driven by a static set of industry heuristics within the `narrativeModel` prompts, or have you implemented a way to steer this translation based on different target role personas (e.g., emphasizing 'platform direction' for Staff roles versus 'system design' for Architects)?
**A:** The mapping is not implemented as a static dictionary of internal terms to industry keywords.

The current approach is prompt-driven. The narrativeModel acts as a separate interpretation layer on top of the technical reconstruction. The earlier stages extract and structure the evidence from Git history, patches, files, and answers. The narrativeModel then turns that evidence into a career-oriented narrative using prompt instructions.

This separation is important because the technical report and the career narrative have different goals. The technical report explains what changed in the repository. The narrative layer explains what that work means in professional terms: architecture ownership, developer tooling, platform direction, production stability, security hardening, migration work, or maintainability.

So the translation is driven by industry heuristics embedded in the narrative prompts, not by a hardcoded lookup table. For example, the model can interpret a group of commits around CI, package publishing, dependency management, and test coverage as developer productivity or project maintainability, but only if the Git evidence supports that interpretation.

At this stage, role steering exists mainly at the prompt level. The system can be guided to emphasize different aspects of the same evidence depending on the target narrative. For a Staff-level narrative, it can emphasize platform direction, ownership, repeatability, and cross-project impact. For an Architect-style narrative, it can emphasize system design, boundaries, trade-offs, and long-term maintainability.

I would not describe this yet as a fully developed persona engine with separate configurable role profiles. It is currently a prompt-based narrative layer that can be steered by role intent. A future version could make this more explicit by adding role profiles such as Staff Engineer, Architect, Tech Lead, or Security Engineer.

The important boundary is that the role narrative cannot invent impact. The target role can change which parts of the evidence are emphasized, but the evidence itself still comes from Git history and the reconstruction pipeline.

**Q:** As you move from MVP to an OSS project, do you intend to evolve the current monolithic pipeline into a plugin-based architecture that allows contributors to define their own custom signal detectors or noise-filtering rules for different repository types?
**A:** (no answer)
