...
import { EMPTY_SUMMARIZE_MODEL, OVERSIZED_SPLIT_MODEL, PATH_FILTER_SKIP_MODEL } from "../../../src/lib/model.js";
import { commitSummaryPath } from "../../../src/lib/grouping.js";
import {
  chatJsonFromSchema,
  FAKE_REPO,
  modelLayerPayload,
...
describe("summarize", () => {
  ...
  it("skips per-part LLM when split exceeds MAX_SUMMARIZE_PARTS", async () => {
    const hash = "abc1234567890abc1234567890abc1234567890";
    ...
    const output = lines.join("\n");
    expect(output).toContain("split mode · 21 parts");
    expect(output).toContain("skipped (21 parts > 20)");
    expect(chatJsonMock).not.toHaveBeenCalled();

    const canonicalPath = commitSummaryPath(repoSummariesDir(FAKE_REPO), timestamp, hash);
    const canonical = JSON.parse(fs.readFileSync(canonicalPath, "utf8")) as {
      model: { summary: string; provenance: { model: string } };
    };
    expect(canonical.model.provenance.model).toBe(OVERSIZED_SPLIT_MODEL);
    expect(canonical.model.summary).toContain("21 evidence parts");
  });

  it("skips LLM for binary/autogenerated bucket parts", async () => {
    const hash = "abc1234567890abc1234567890abc1234567890";
    seedSplitCommitWithBucket(FAKE_REPO, hash);
    ...
    const output = lines.join("\n");
    expect(output).toContain("split mode · 3 parts");
    expect(output).toContain("skipped (binary/autogenerated bucket)");

    const part3Path = commitSummaryPartPath(
      repoSummariesDir(FAKE_REPO),
      1_700_000_000,
      hash,
      3,
    );
    const part3 = JSON.parse(fs.readFileSync(part3Path, "utf8")) as {
      model: { summary: string; provenance: { model: string } };
    };
    expect(part3.model.provenance.model).toBe(PATH_FILTER_SKIP_MODEL);
    expect(part3.model.summary).toContain("auto-generated");
  });
