...
import { writeMigrationJson } from "../helpers/migration-fixtures.js";
import {
...
describe("pipeline provenance migration v1000005", () => {
  ...
  it("migrates group → report → prepared chain with provenance", async () => {
    ...
    writeMigrationJson(path.join(summariesDir, "100", "abc.json"), {
      commitHash: "abc",
      timestamp: 100,
      sourceEvidence: "100",
      model,
    ...
    writeMigrationJson(path.join(groupsDir, `${groupId}.json`), {
      groupId,
    ...
    writeMigrationJson(path.join(reportsDir, `${groupId}.json`), {
      reportId: groupId,
    ...
    writeMigrationJson(path.join(preparedDir, `${groupId}.json`), {
      preparedId: groupId,
    ...
    writeMigrationJson(path.join(finishDir, `${groupId}.question.json`), {
    ...
    writeMigrationJson(path.join(finishDir, `${groupId}.json`), {
      finishId: groupId,
  ...
  it("migrates legacy group schema 1000003 with string signal reasons and question cards", () => {
    ...
    writeMigrationJson(path.join(summariesDir, "1783577173", `${commitHash}.json`), {
      commitHash,
