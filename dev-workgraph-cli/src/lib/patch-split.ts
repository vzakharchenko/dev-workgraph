...
/** All repository-relative paths referenced in a patch (from `diff --git` lines). */
export function listPatchFilePaths(patch: string): string[] {
  return collectPaths(patch);
}

/**
 * Splits a patch into authored hunks vs paths classified as binary/generated.
 * Whole hunks move to the bucket when any of their paths is excluded.
 * @param patch - Full patch text.
 * @param excludePaths - Paths to peel off (binary or auto-generated by filename).
 */
export function peelPatchByPaths(
  patch: string,
  excludePaths: ReadonlySet<string>,
): { authoredPatch: string; bucketPaths: string[] } {
  if (excludePaths.size === 0) {
    return { authoredPatch: patch, bucketPaths: [] };
  }

  const { header, hunks } = splitPatchByFile(patch);
  const authored: FileHunk[] = [];
  const bucket: string[] = [];

  for (const hunk of hunks) {
    if (hunk.paths.some((p) => excludePaths.has(p))) {
      for (const p of hunk.paths) {
        if (excludePaths.has(p)) bucket.push(p);
      }
    } else {
      authored.push(hunk);
    }
  ...
  const authoredBody = authored.map((h) => h.text).join("");
  const authoredPatch = authored.length > 0 ? `${header}${authoredBody}` : header;
  return { authoredPatch, bucketPaths: uniqPaths(bucket) };
}

/**
 * Minimal patch body for a binary/autogenerated bucket (path manifest only — no diff hunks).
 * @param header - Commit metadata lines before the first `diff --git`.
 * @param paths - Repository-relative paths in the bucket.
 */
export function buildBucketManifestPatch(header: string, paths: string[]): string {
  const lines = [
    header.trimEnd(),
    "",
    "# Paths classified as likely binary or auto-generated by filename.",
    "# Diff content omitted — not analyzed as authored code.",
    ...paths.map((p) => `# ${p}`),
  ];
  return `${lines.filter((l, i) => i > 0 || l.length > 0).join("\n")}\n`;
}

/** Commit metadata lines before the first `diff --git` hunk. */
export function patchCommitHeader(patch: string): string {
  return splitPatchByFile(patch).header;
}
