...
export interface CommitEvidenceRecord {
  ...
  partCount?: number;
  /** True when LLM path filter peeled binary/generated paths into a bucket part. */
...
export interface CommitEvidencePartRecord {
  ...
  /** When true, summarize skips the LLM and writes a predefined low-signal layer. */
  isBinaryOrAutogenerated?: boolean;
  deterministic: DeterministicLayer;
}
