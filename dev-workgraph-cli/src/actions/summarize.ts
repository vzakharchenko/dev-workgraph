...
async function summarizeOnePart(input: {
  ...
  if (partEvidence.isBinaryOrAutogenerated) {
    const layer = binaryOrAutogeneratedModelLayer(partEvidence.deterministic);
    ...
    console.log("skipped (binary/autogenerated bucket)");
    return layer;
  }
...
function handleOversizedSplitCommit(ctx: SplitCommitContext): void {
  console.log(`  skipped (${ctx.partCount} parts > ${MAX_SUMMARIZE_PARTS})`);
  writeSplitCanonicalAndDone(ctx, oversizedSplitModelLayer(ctx.partCount));
}
...
async function summarizeSplitCommit(input: SplitCommitContext): Promise<void> {
  ...
  if (input.partCount > MAX_SUMMARIZE_PARTS) {
    handleOversizedSplitCommit(input);
    return;
  }
